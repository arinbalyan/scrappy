...
func TestWriteCSV(t *testing.T) {
	...
	if err := WriteCSV(out, jobs); err != nil {
	...
	if !strings.Contains(content, "Software Engineer") {
		t.Fatalf("expected job row in csv, got: %s", content)
	}
	if !strings.Contains(content, "hr@acme.com") {
		t.Fatalf("expected email in csv, got: %s", content)
	}

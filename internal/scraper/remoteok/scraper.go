...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil { return nil, fmt.Errorf("remoteok request: %w", err) }
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 { return nil, fmt.Errorf("remoteok status %d", resp.StatusCode) }

	var raw []map[string]any
	if err := json.NewDecoder(resp.Body).Decode(&raw); err != nil { return nil, fmt.Errorf("remoteok decode: %w", err) }
	if len(raw) > 0 { raw = raw[1:] }

	limit := input.ResultsWanted
	if limit <= 0 || limit > len(raw) { limit = len(raw) }
	...
	for i := 0; i < limit; i++ {
		...
		if epoch, ok := j["epoch"].(float64); ok { t := time.Unix(int64(epoch), 0); posted = &t }

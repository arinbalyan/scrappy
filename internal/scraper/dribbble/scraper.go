...
const feedURL = "https://dribbble.com/jobs.rss"

var drStripTags = regexp.MustCompile(`(?is)<[^>]+>`)
...
func (s *Scraper) SiteName() model.Site { return model.SiteDribbble }
...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dribbble request: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("dribbble status %d", resp.StatusCode)
	}
	...
	for {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("dribbble decode: %w", err)
		}
		...
		job := model.JobPost{
			ID:          "dribbble-" + idFromURL(firstNonEmpty(it.GUID, link)),
			Title:       title,
			JobURL:      link,
	...
	if !util.HasMeaningfulJobs(jobs) {
		return nil, fmt.Errorf("dribbble no parseable jobs")
	}
	return jobs, nil
}
...
func htmlToText(v string) string {
	...
	v = drStripTags.ReplaceAllString(v, " ")

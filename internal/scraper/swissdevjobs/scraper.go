...
var (
	...
	// salaryRegex matches "[CHF min - max]" or "[EUR min - max]" in the title.
	salaryRegex = regexp.MustCompile(`\[(?:CHF|EUR)\s*([\d']+)\s*-\s*([\d']+)\]`)
	// companyRegex matches "@ Company [" in the title.
	companyRegex = regexp.MustCompile(`@\s*(.+?)\s*\[`)
)

// Scraper fetches jobs from the SwissDevJobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new SwissDevJobs scraper.
func New(client *http.Client) *Scraper {
	if client == nil {
		client = util.NewHTTPClient(util.ClientOptions{Timeout: 15 * time.Second})
	}
...
// Scrape fetches jobs from the SwissDevJobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("swissdevjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("swissdevjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("swissdevjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		// Client-side search term filtering on title + description + category
		if term != "" {
		...
		job := model.JobPost{
			ID:     "swissdevjobs-" + extractID(item.guid, link),
			Title:  title,
			JobURL: link,
			Site:   string(s.SiteName()),
		}

		// Location: always Switzerland
		job.Location.Country = "Switzerland"
		...
		// Parse salary from title format "[CHF min - max]"
		if m := salaryRegex.FindStringSubmatch(title); len(m) == 3 {
		...
		if m := companyRegex.FindStringSubmatch(title); len(m) == 2 {

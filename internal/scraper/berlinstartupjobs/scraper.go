...
// Scraper fetches jobs from the Berlin Startup Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Berlin Startup Jobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SiteBerlinStartupJobs }

// Scrape fetches jobs from the Berlin Startup Jobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("berlinstartupjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("berlinstartupjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("berlinstartupjobs: status %d — try using --proxy with a residential proxy", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("berlinstartupjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("berlinstartupjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		if parts := strings.Split(title, "//"); len(parts) > 1 {
			companyName = strings.TrimSpace(parts[len(parts)-1])
		}

		job := model.JobPost{
			ID:          "berlinstartupjobs-" + extractID(item.guid, item.link),
			Title:       title,
			CompanyName: companyName,
			JobURL:      link,
			Description: strings.TrimSpace(item.description),
			Location:    model.Location{City: "Berlin"},
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("berlinstartupjobs: no parseable jobs")
	}
	return out, nil
}

...
func TestScraper_SiteName(t *testing.T) {
	s := New(nil)
	if got := s.SiteName(); got != model.SiteRecruitee {
		t.Errorf("SiteName() = %q, want %q", got, model.SiteRecruitee)
	}
}

func TestScraper_Scrape(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := recruiteeResponse{
	...
	j1 := result[0]
	if !strings.HasPrefix(j1.ID, "recruitee-") {
		t.Errorf("job[0].ID = %q, expected recruitee- prefix", j1.ID)
	}

...
func TestScraper_SiteName(t *testing.T) {
	s := New(nil)
	if got := s.SiteName(); got != model.SitePinpoint {
		t.Errorf("SiteName() = %q, want %q", got, model.SitePinpoint)
	}
}
...
func TestScraper_Scrape(t *testing.T) {
	...
	j1 := result[0]
	if !strings.HasPrefix(j1.ID, "pinpoint-") {
		t.Errorf("job[0].ID = %q, expected pinpoint- prefix", j1.ID)
	}

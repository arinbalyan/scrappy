...
const feedURL = "https://www.golangprojects.com/rss.xml"
...
// Scraper fetches jobs from the Golang Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Golang Jobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SiteGolangJobs }

// Scrape fetches jobs from the Golang Jobs RSS feed. It uses regex-based XML
...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("golangjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("golangjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("golangjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("golangjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("golangjobs: parsed items", map[string]any{"count": len(items)})
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("golangjobs: no parseable jobs")
	}
	return out, nil
}

...
const feedURL = "https://www.python.org/jobs/feed/rss/"
...
// Scraper fetches jobs from the Python Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Python Jobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SitePythonJobs }

// Scrape fetches jobs from the Python Jobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pythonjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("pythonjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("pythonjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("pythonjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("pythonjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		job := model.JobPost{
			ID:     "pythonjobs-" + extractID(item.guid, link),
			Title:  title,
			JobURL: link,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("pythonjobs: no parseable jobs")
	}
	return out, nil
}

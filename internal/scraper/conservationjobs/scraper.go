...
// Scraper fetches jobs from the Conservation Job Board RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Conservation Jobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SiteConservationJobs }

// Scrape fetches jobs from the Conservation Job Board RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("conservationjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("conservationjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("conservationjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("conservationjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("conservationjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		job := model.JobPost{
			ID:          "conservationjobs-" + extractID(item.guid, item.link),
			Title:       title,
			JobURL:      link,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("conservationjobs: no parseable jobs")
	}
	return out, nil
}

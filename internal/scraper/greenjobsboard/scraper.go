...
const feedURL = "https://greenjobs.greenjobsearch.org/rss"
...
// Scraper fetches jobs from the Green Jobs Board RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Green Jobs Board scraper. If client is nil a default one is used.
...
func (s *Scraper) SiteName() model.Site { return model.SiteGreenJobsBoard }

// Scrape fetches jobs from the Green Jobs Board RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("greenjobsboard: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("greenjobsboard: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("greenjobsboard: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("greenjobsboard: read: %w", err)
	}

	items := parseRSSItems(string(body))
	util.Debug("greenjobsboard: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		// Client-side search term filtering on title, description, and creator
		if term != "" {
			hay := strings.ToLower(title + " " + item.description + " " + item.creator)
		...
		desc := strings.TrimSpace(item.contentEncoded)
		if desc == "" {
			desc = strings.TrimSpace(item.description)
		}

		job := model.JobPost{
			ID:          "greenjobsboard-" + extractID(item.guid, link),
			Title:       title,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("greenjobsboard: no parseable jobs")
	}
	return out, nil
}
...
func extractTag(xml, tag string) string {
	...
	// Tags with colons (e.g. dc:creator, content:encoded) need escaped colons
	tagEscaped := strings.ReplaceAll(tag, ":", `:?`)
	// Try plain content
	rx = regexp.MustCompile(fmt.Sprintf(`(?is)<%s[^>]*>([\s\S]*?)</%s>`, tagEscaped, tagEscaped))
	if m := rx.FindStringSubmatch(xml); len(m) == 2 {

...
func TestScraper_SiteName(t *testing.T) {
	s := New(nil)
	if got := s.SiteName(); got != model.SiteTaleo {
		t.Errorf("SiteName() = %q, want %q", got, model.SiteTaleo)
	}
}

func TestScraper_Scrape(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := taleoSearchResponse{
	...
	j1 := result[0]
	if !strings.HasPrefix(j1.ID, "taleo-") {
		t.Errorf("job[0].ID = %q, expected taleo- prefix", j1.ID)
	}

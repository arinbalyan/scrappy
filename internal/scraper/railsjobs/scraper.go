...
const feedURL = "https://jobs.rubyonrails.org/jobs.rss"
...
// Scraper fetches jobs from the Rails Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Rails Jobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SiteRailsJobs }

// Scrape fetches jobs from the Rails Jobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("railsjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("railsjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("railsjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("railsjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("railsjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		job := model.JobPost{
			ID:     "railsjobs-" + extractID(item.guid, link),
			Title:  title,
			JobURL: link,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("railsjobs: no parseable jobs")
	}
	return out, nil
}

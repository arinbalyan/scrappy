...
const defaultURL = "https://ml-jobs.com/jobs"
...
func (s *Scraper) SiteName() model.Site { return model.SiteMLJobs }
...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mljobs request: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("mljobs status %d", resp.StatusCode)
	}
	b, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("mljobs read: %w", err)
	}
	...
	for i, row := range m {
		...
		out = append(out, model.JobPost{ID: fmt.Sprintf("mljobs-%d", i+1), Title: title, JobURL: u, IsRemote: true})
		if input.ResultsWanted > 0 && len(out) >= input.ResultsWanted {
			break
		}
	}
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("mljobs no parseable jobs")
	}
	return out, nil
}

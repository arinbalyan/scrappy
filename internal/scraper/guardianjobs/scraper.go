...
const feedURL = "https://jobs.theguardian.com/jobsrss/"

var (
	itemRe   = regexp.MustCompile(`(?is)<item>(.*?)</item>`)
	cdataRe  = regexp.MustCompile(`(?is)^\s*<!\[CDATA\[(.*?)\]\]>\s*$`)
	idNumber = regexp.MustCompile(`/job/(\d+)/`)
)

// Scraper fetches jobs from the Guardian Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Guardian Jobs scraper. If client is nil a default one is used.
...
func (s *Scraper) SiteName() model.Site { return model.SiteGuardianJobs }

// Scrape fetches jobs from the Guardian Jobs RSS feed.
...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("guardianjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("guardianjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("guardianjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("guardianjobs: read: %w", err)
	}

	items := parseRSSItems(string(body))
	util.Debug("guardianjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		if idx := strings.Index(title, ": "); idx > 0 {
			companyName = strings.TrimSpace(title[:idx])
			jobTitle = strings.TrimSpace(title[idx+2:])
		}

		job := model.JobPost{
			ID:          "guardianjobs-" + extractID(item.guid, link),
			Title:       jobTitle,
			CompanyName: companyName,
			JobURL:      link,
			Description: strings.TrimSpace(item.description),
			Site:        string(s.SiteName()),
			Location:    model.Location{Country: "UK"},
		}
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("guardianjobs: no parseable jobs")
	}
	return out, nil
}
...
// extractID extracts a short ID from a URL. Guardian Jobs uses /job/NNN/ format.
func extractID(guid, link string) string {
	...
	if m := idNumber.FindStringSubmatch(raw); len(m) == 2 {
		return m[1]
	}

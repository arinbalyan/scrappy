...
const feedURL = "https://iosdevjobs.com/jobs.rss"
...
// Scraper fetches jobs from the iOS Dev Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new iOS Dev Jobs scraper. If client is nil a default one is used.
...
// Scrape fetches jobs from the iOS Dev Jobs RSS feed.
...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("iosdevjobs: build request: %w", err)
	}
	...
	util.Debug("iosdevjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		jobTitle, companyName := parseTitle(title)

		job := model.JobPost{
			ID:          "iosdevjobs-" + extractID(link),
			Title:       jobTitle,
...
// parseTitle splits "Title @ Company" into job title and company name.
func parseTitle(title string) (jobTitle, companyName string) {
	atIdx := strings.LastIndex(title, " @ ")
	if atIdx > 0 {
		return strings.TrimSpace(title[:atIdx]), strings.TrimSpace(title[atIdx+3:])
	}
	return title, ""
}

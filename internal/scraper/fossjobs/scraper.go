...
const feedURL = "https://www.fossjobs.net/rss/all/"
...
// Scraper fetches jobs from the FossJobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new FossJobs scraper. If client is nil a default one is used.
...
func (s *Scraper) SiteName() model.Site { return model.SiteFossJobs }

// Scrape fetches jobs from the FossJobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fossjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("fossjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("fossjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("fossjobs: read: %w", err)
	}
	...
	for _, item := range items {
		...
		job := model.JobPost{
			ID:          "fossjobs-" + extractID(item.guid, link),
			Title:       title,
			JobURL:      link,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("fossjobs: no parseable jobs")
	}
	return out, nil
}

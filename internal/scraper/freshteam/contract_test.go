...
func TestScraper_SiteName(t *testing.T) {
	s := New(nil)
	if got := s.SiteName(); got != model.SiteFreshteam {
		t.Errorf("SiteName() = %q, want %q", got, model.SiteFreshteam)
	}
}

func TestScraper_Scrape(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		postings := []freshteamJobPosting{
			{
				ID:          1,
	...
	j1 := result[0]
	if !strings.HasPrefix(j1.ID, "freshteam-") {
		t.Errorf("job[0].ID = %q, expected freshteam- prefix", j1.ID)
	}
...
func TestScraper_Scrape_EmptyBody(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode([]freshteamJobPosting{})
	}))
	defer ts.Close()

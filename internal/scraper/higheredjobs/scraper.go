...
const feedURL = "https://www.higheredjobs.com/search/rss.cfm"
...
// Scraper fetches jobs from the HigherEdJobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new HigherEdJobs scraper. If client is nil a default one is used.
...
func (s *Scraper) SiteName() model.Site { return model.SiteHigherEdJobs }

// Scrape fetches jobs from the HigherEdJobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("higheredjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("higheredjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("higheredjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("higheredjobs: read: %w", err)
	}

	items := parseRSSItems(string(body))
	util.Debug("higheredjobs: parsed items", map[string]any{"count": len(items)})
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("higheredjobs: no parseable jobs")
	}
	return out, nil
}
...
// extractIDFromURL extracts a short ID from a URL using the last path segment.
func extractIDFromURL(raw string) string {
	...
	// Remove query string for cleaner IDs

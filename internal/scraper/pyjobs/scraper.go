...
const feedURL = "https://www.pyjobs.com/rss"
...
// Scraper fetches jobs from the PyJobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new PyJobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SitePyJobs }

// Scrape fetches jobs from the PyJobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pyjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("pyjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("pyjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("pyjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("pyjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		job := model.JobPost{
			ID:     "pyjobs-" + extractID(item.guid, link),
			Title:  title,
			JobURL: link,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("pyjobs: no parseable jobs")
	}
	return out, nil
}

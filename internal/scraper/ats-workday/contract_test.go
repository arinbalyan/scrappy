...
func TestScraper_SiteName(t *testing.T) {
	s := New(nil)
	if got := s.SiteName(); got != model.SiteWorkday {
		t.Errorf("SiteName() = %q, want %q", got, model.SiteWorkday)
	}
}

func TestScraper_Scrape(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := wdSearchResponse{
			Total: 2,
			JobPostings: []wdJobListItem{
				{
					Title:        "Software Engineer",
					ExternalPath: "/en-US/External/job/1001",
				...
				},
				{
					...
					ExternalPath:  "/en-US/External/job/1002",
	...
	j1 := result[0]
	if !strings.HasPrefix(j1.ID, "wd-") {
		t.Errorf("job[0].ID = %q, expected wd- prefix", j1.ID)
	}
	if j1.Title != "Software Engineer" {
		t.Errorf("job[0].Title = %q", j1.Title)
	}
	if j1.CompanyName != "tesla" {
...
func TestScraper_Scrape_HTTPError(t *testing.T) {
	...
	s := NewWithAPIURL(nil, ts.URL)
	_, err := s.Scrape(context.Background(), model.ScraperInput{SearchTerm: "tesla:5:Tesla"})
	if err == nil {

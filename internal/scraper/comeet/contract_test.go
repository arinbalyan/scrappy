...
func TestScraper_SiteName(t *testing.T) {
	s := New(nil)
	if got := s.SiteName(); got != model.SiteComeet {
		t.Errorf("SiteName() = %q, want %q", got, model.SiteComeet)
	}
}

func TestScraper_Scrape(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		listings := []comeetListing{
			{
				UID:          "pos-1",
				Name:         "Software Engineer",
				CompanyName:  "Acme Corp",
				URLActivePage: "https://acme.comeet.com/jobs/pos-1",
				Location:     &comeetLocation{Name: "San Francisco, CA"},
				Department:   "Engineering",
				TimeUpdated:  "2025-01-15T00:00:00Z",
			},
			{
				UID:         "pos-2",
				Name:        "Product Manager",
				CompanyName: "Acme Corp",
				Location:    &comeetLocation{Name: "Remote"},
				Department:  "Product",
				TimeUpdated: "2025-01-14T00:00:00Z",
			},
		}
	...
	j1 := result[0]
	if !strings.HasPrefix(j1.ID, "comeet-") {
		t.Errorf("job[0].ID = %q, expected comeet- prefix", j1.ID)
	}
...
func TestScraper_Scrape_EmptyBody(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode([]comeetListing{})
	}))
	defer ts.Close()

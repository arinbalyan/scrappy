...
const feedURL = "https://opensourcedesign.net/jobs/feed.xml"
...
// Scraper fetches jobs from the Open Source Design Jobs RSS feed.
type Scraper struct {
	client  *http.Client
	feedURL string
}

// New creates a new Open Source Design Jobs scraper.
...
func (s *Scraper) SiteName() model.Site { return model.SiteOpenSourceDesignJobs }

// Scrape fetches jobs from the Open Source Design Jobs RSS feed.
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("opensourcedesignjobs: build request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("opensourcedesignjobs: request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("opensourcedesignjobs: status %d", resp.StatusCode)
	}

	body, err := util.ReadBodyLimited(resp.Body, util.DefaultMaxBodyBytes)
	if err != nil {
		return nil, fmt.Errorf("opensourcedesignjobs: read: %w", err)
	}

	items := parseItems(string(body))
	util.Debug("opensourcedesignjobs: parsed items", map[string]any{"count": len(items)})
	...
	for _, item := range items {
		...
		job := model.JobPost{
			ID:     "opensourcedesignjobs-" + extractID(item.guid, link),
			Title:  title,
			JobURL: link,
	...
	if !util.HasMeaningfulJobs(out) {
		return nil, fmt.Errorf("opensourcedesignjobs: no parseable jobs")
	}
	return out, nil
}

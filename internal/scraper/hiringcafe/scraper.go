...
const defaultAPI = "https://hiring.cafe/api/jobs"
...
func (s *Scraper) Scrape(ctx context.Context, input model.ScraperInput) ([]model.JobPost, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&parsed); err != nil {
		return nil, fmt.Errorf("hiringcafe decode: %w", err)
	}
	...
	for i := 0; i < limit; i++ {
		...
		jobs = append(jobs, model.JobPost{ID: "hc-" + strings.TrimSpace(r.ID), Title: strings.TrimSpace(r.Title), CompanyName: strings.TrimSpace(r.Company), Location: model.Location{City: strings.TrimSpace(r.Location)}, JobURL: strings.TrimSpace(r.URL), Description: strings.TrimSpace(r.Description), DatePosted: posted, IsRemote: r.Remote || strings.Contains(strings.ToLower(r.Location), "remote")})
	}
	return jobs, nil
}

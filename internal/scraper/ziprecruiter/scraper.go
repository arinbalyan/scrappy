...
var (
	reJob      = regexp.MustCompile(`(?s)data-job-id="([^"]+)"[\s\S]*?<a[^>]*class="job_content"[^>]*href="([^"]+)"[\s\S]*?<h2[^>]*>([^<]+)</h2>[\s\S]*?<a[^>]*class="t_org_link"[^>]*>([^<]+)</a>`)
...
func parseHTMLJobs(raw string) []model.JobPost {
	...
	for _, row := range m {
		out = append(out, model.JobPost{
			ID:          "zr-" + row[1],
			JobURL:      strings.TrimSpace(row[2]),
			Title:       strings.TrimSpace(row[3]),
			CompanyName: strings.TrimSpace(row[4]),
		})
	}
	return out
}

...
4. An ASCII-art logo prints, followed by the version and a prompt.
...
(Printed in `\033[38;5;117m` — light blue on supported terminals.)
...
If found, the `defaults:` block populates wizard prompts (search, location, results-wanted, format, out, etc.). If not found, an empty default is used.
...
Caps internal concurrency to stay within budget.
...
If you answer `y` or `yes`, the current wizard values are written to `~/.scrappy/config.yaml`. This makes them the defaults for future runs.
...
```
  Some sites require API keys to function:
    ...
    ✗ infojobs        INFOJOBS_CLIENT_ID, INFOJOBS_CLIENT_SECRET

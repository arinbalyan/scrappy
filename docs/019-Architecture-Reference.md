...
scrappy scrape (CLI or Go library)
  ...
  +-- [proxy/]    Proxy pool + TCP-dial health probe + SOCKS5/HTTP
  +-- [scraper/*] Site scrapers (concurrent, bounded by per-site semaphores)
  |     |
  |     +-- [email/]       Email extractor + MX validator + company-page enrich
  |     +-- [normalize/]   Salary normalization, title normalization
  +-- [dedup/]    URL deduplicator + company dedup (thread-safe Set)
  +-- [quality/]  Deterministic score 0-100 (concurrent maps/atomics)
  +-- [export/]   Writer -> csv / xlsx / jsonl / parquet
```

## Data flow

1. CLI or library call creates a `ScraperInput` with site list, search term, filters, rate limits, and memory cap.
...
5. Email domains populate `JobPost.Domain` for quality scoring.
6. Salary normalization converts all compensation to annual USD.
7. Deduplicator removes cross-site duplicate URLs and by-company duplicates.
8. Quality scorer adds a 0-100 score to each posting using concurrent maps with atomic counters.
9. Exporter streams filtered results to the chosen format via Go channels -- no full in-memory materialization required for Parquet/JSONL.
...
locs   = ["Remote", "New York"]
       |
passes = (AI Engineer x Remote), (AI Engineer x New York),
         (Software Developer x Remote), (Software Developer x New York)
```
...
raw JobPost from scraper
  ...
  +- 3. enrichJobEmails (regex extract + MX verify + dedup + domain population)
  +- 4. quality.Score (compute 0-100, thread-safe via concurrent maps/atomics)
...
| `internal/scraper/` | `Scraper` interface + 62 site-specific scraper packages |
...
The entire codebase is tested with `go test -race ./...`. Key concurrency patterns:

- **Quality scoring**: Uses `sync.Map` for domain-aggregation lookups and `sync/atomic` for counter increments across goroutines.
- **Dedup sets**: Protected by `sync.Mutex` for safe concurrent read/write.
- **Scraper goroutines**: Bounded by `errgroup.Group` with per-site semaphores, preventing runaway goroutine creation.
- **Context-aware sleep**: `util.SleepWithContext(ctx, duration)` replaces `time.Sleep()` in all scrapers, ensuring timely cancellation when a scrape is terminated. This prevents goroutine leaks in cancellation scenarios.

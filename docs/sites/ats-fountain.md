...
## Current integration
- Parser order:
  1. ATS API: https://api.fountain.com/v2/openings
- Extracted fields: title, company, location, description, site identifier

## Supported knobs
- `results_wanted`
- shared resilience knobs (`retries`, `max_rps`, `site_rps`, proxy config)
- Company slug seed via `SCRAPPY_FOUNTAIN_SEEDS` env var, `config/company_slugs.yaml`, or `--search` as fallback

## Seed resolution
- `ats.ResolveSeeds` with key `SCRAPPY_FOUNTAIN_SEEDS`
...
- Fountain ATS API. Requires FOUNTAIN_API_KEY env var as Bearer token. Returns job openings with structured location and compensation.

## Debug/update playbook
1. Verify API endpoint is reachable and returns expected JSON.
2. Check that `SCRAPPY_FOUNTAIN_SEEDS` or the config file has target company slugs.
